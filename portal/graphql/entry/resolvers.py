...
def resolve_entry(info, global_entry_id=None, slug=None):
    ...
    if global_entry_id:
        ...
        entry = models.Entry.published.visible_to_user(user).filter(pk=entry_pk).first()
    else:
        entry = models.Entry.published.visible_to_user(user).filter(slug=slug).first()
    return entry


def resolve_entries(info):
    user = info.context.user
    return models.Entry.published.visible_to_user(user)
