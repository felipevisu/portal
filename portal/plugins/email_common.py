...
def validate_default_email_configuration(
    ...
    if configuration["use_tls"] and configuration["use_ssl"]:
        ...
        raise ValidationError(
            {
                "use_ssl": ValidationError(error_msg),
                "use_tls": ValidationError(error_msg),
            }
        )
    config = EmailConfig(
    ...
    if not config.sender_address:
        raise ValidationError(
            {"sender_address": ValidationError("Missing sender address value.")}
        )

    EmailValidator(
        message={  # type: ignore[arg-type] # the code below is a hack
            "sender_address": ValidationError("Invalid email")
        }
    ...
    try:
        ...
        raise ValidationError(
            {c: ValidationError(error_msg) for c in asdict(config).keys()}
        )
